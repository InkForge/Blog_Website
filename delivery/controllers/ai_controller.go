...
func (c0 *AIController) SuggestTags(c *gin.Context) {
    if c0.aiUsecase == nil {
    ...
    defer cancel()
    tags, err := c0.aiUsecase.SuggestTags(ctx, req.Title, req.Content, req.MaxTags)
    if err != nil {
...
func (c0 *AIController) Summarize(c *gin.Context) {
    if c0.aiUsecase == nil {
        c.JSON(http.StatusServiceUnavailable, dto.ErrorResponse{Error: "ServiceUnavailable", Message: "AI service not configured", Code: http.StatusServiceUnavailable})
        return
    }
    ...
    ctx, cancel := context.WithTimeout(c.Request.Context(), 20*time.Second)
    defer cancel()
    summary, err := c0.aiUsecase.Summarize(ctx, req.Content, req.MaxWords)
    if err != nil {
...
func (c0 *AIController) GenerateTitle(c *gin.Context) {
    if c0.aiUsecase == nil {
        c.JSON(http.StatusServiceUnavailable, dto.ErrorResponse{Error: "ServiceUnavailable", Message: "AI service not configured", Code: http.StatusServiceUnavailable})
        return
    }
    ...
    ctx, cancel := context.WithTimeout(c.Request.Context(), 20*time.Second)
    defer cancel()
    title, err := c0.aiUsecase.GenerateTitle(ctx, req.Content, req.Style)
    if err != nil {
...
func (c0 *AIController) SuggestContent(c *gin.Context) {
    if c0.aiUsecase == nil {
        c.JSON(http.StatusServiceUnavailable, dto.ErrorResponse{Error: "ServiceUnavailable", Message: "AI service not configured", Code: http.StatusServiceUnavailable})
        return
    }
    ...
    defer cancel()
    content, err := c0.aiUsecase.SuggestContent(ctx, req.Keywords, req.Style, req.WordCount)
    if err != nil {
...
func (c0 *AIController) ImproveContent(c *gin.Context) {
    if c0.aiUsecase == nil {
        c.JSON(http.StatusServiceUnavailable, dto.ErrorResponse{Error: "ServiceUnavailable", Message: "AI service not configured", Code: http.StatusServiceUnavailable})
        return
    }
    ...
    ctx, cancel := context.WithTimeout(c.Request.Context(), 25*time.Second)
    defer cancel()
    res, err := c0.aiUsecase.ImproveContent(ctx, req.Content, req.Focus)
    if err != nil {
...
func (c0 *AIController) Chat(c *gin.Context) {
    if c0.aiUsecase == nil {
        c.JSON(http.StatusServiceUnavailable, dto.ErrorResponse{Error: "ServiceUnavailable", Message: "AI service not configured", Code: http.StatusServiceUnavailable})
        return
    }
    ...
    defer cancel()
    out, err := c0.aiUsecase.Chat(ctx, msgs)
    if err != nil {
